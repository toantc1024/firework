...
function init() {
	...
	function setOptionsForSelect(node, options) {
		node.innerHTML = options.reduce((acc, opt) => acc += `<option ${opt.value} value="${opt.value}">${opt.label}</option>`, '');
	}
	...
	// shell size
	options = '';
	['3"', '4"', '6"', '8"', '12"', '16"'].forEach((opt, i) => options += `<option id="val_${i}"" value="${i}">${opt}</option>`);
...
function check(e) {
	...
	let type_shell = "Floral";
    switch (e.key) {
		case "1":
		  type_shell = "Crackle";
		  break;
		case "2": 
		  type_shell = "Crossette";
		  break;	  
		case "3": 
			type_shell = "Crysanthemum";
			break;
		case "4": 
		 	type_shell = "Falling Leaves";
		  	break; 
		case "5": 
			type_shell = "Floral";
			break;
		...
		case "q":
			...
			document.getElementById("hide").click();
			document.getElementById("i-shell").click();
			console.log(document.getElementById("val_0").click());
		default:
		  return; 
	  }
	const shell = new Shell(shellTypes[type_shell](5));
	const w = mainStage.width;
	const h = mainStage.height;
	shell.launch(
		0 ? 0 / w : getRandomShellPositionH(),
		0 ? 1 / h : getRandomShellPositionV()
	);
}
// Launches a shell from a user pointer event, based on state.config
/*function launchShellFromConfig(event) {
	//alert(localStorage.getItem("Type_FLower"));
	const shell = new Shell(shellTypes[localStorage.getItem("Type_Flower")](5));
	const w = mainStage.width;
	const h = mainStage.height;
